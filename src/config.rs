...
impl Config {
    ...
    pub fn source_enabled(&self, kind: SourceKind) -> bool {
        match kind {
            ...
            SourceKind::Pipx => self.sources.pipx,
            SourceKind::Uv => self.sources.uv,
            SourceKind::Scoop => self.sources.scoop,
            SourceKind::Cargo => self.sources.cargo,
            SourceKind::Go => self.sources.go,
    ...
    pub fn set_source_enabled(&mut self, kind: SourceKind, enabled: bool) {
        let slot = match kind {
            ...
            SourceKind::Pipx => &mut self.sources.pipx,
            SourceKind::Uv => &mut self.sources.uv,
            SourceKind::Scoop => &mut self.sources.scoop,
            SourceKind::Cargo => &mut self.sources.cargo,
            SourceKind::Go => &mut self.sources.go,
