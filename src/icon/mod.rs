...
pub fn tray(state: IconState, frame: u32) -> Result<Icon> {
    Ok(
        Icon::from_rgba(render::rgba(state, frame, TRAY_SIZE), TRAY_SIZE, TRAY_SIZE)
...
pub fn write_app_icon(path: &Path) -> std::io::Result<()> {
    let images: Vec<(u32, Vec<u8>)> = ico::SIZES
        .iter()
        .map(|size| (*size, render::rgba(IconState::Idle, 0, *size)))
        .collect();
...
mod tests {
    ...
    fn dump_every_state_for_visual_review() {
        ...
        let mut sheet = vec![
            ("idle", IconState::Idle, 0),
            ("updates", IconState::Updates, 0),
            ("error", IconState::Error, 0),
        ];
        for frame in 0..BUSY_FRAMES {
            sheet.push(("busy", IconState::Busy, frame));
        }

        for (label, state, frame) in sheet {
            let images: Vec<(u32, Vec<u8>)> = ico::SIZES
                .iter()
                .map(|size| (*size, render::rgba(state, frame, *size)))
                .collect();
